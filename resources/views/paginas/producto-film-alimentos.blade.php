        ...
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb text-uppercase py-2 font-size-13">
                <li class="breadcrumb-item">
                    <a href="{{ route('filmStrech') }}" class=" text-decoration-none font-weight-600" style="color: black;">Film de Alimentos</a>
                </li>         
                ...
                <ul class="p-0" style="list-style: none;">
                    @foreach ($products as $p)
                        <li class="py-2 @if($p->id == $product->id) active @endif" style="border-top: 1px solid #E9E9E9;
                            ...
                            <a href="{{ route('productoFilmAlimentos', ['producto' => $p->id]) }}" class="d-block p-2 text-decoration-none text-dark fw-600 font-size-14 text-uppercase">{{$p->name}}</a>
                        </li>                        
                    @endforeach
                </ul>             
            </aside>
                ...
                <div class="row mb-5">
                                ...
                                @if (count($product->images))
                                    @foreach ($product->images as $k => $pi)
                                        <div class="carousel-item  @if(!$k) active @endif">
                                            <img src="{{ asset($pi->image) }}" class="mx-auto d-block img-fluid" alt="{{$product->name}}">
                                        </div>                                    
                                    @endforeach
                                @else 
                    ...
                    <div class="col-sm-12 col-md-6">
                        ...
                        @if (count($product->variableProducts))
                            <div class="table-responsive mb-sm-3 mb-md-5">
                                <table>
                                    @foreach ($product->variableProducts as $vp)
                                        <tr>
                                            <td class="px-2"><strong class="fw-bold">Ancho:</strong> {{$vp->width}}</td>
                                            <td class="px-2"><strong class="fw-bold">Largo:</strong> {{$vp->long}}</td>
                                        </tr>
                                    @endforeach                     
                                </table>
                            </div>
                        ...
                        <div class="row">
                            @foreach ($rps as $rp)
                                <div class="col-sm-12 col-md-4 mb-4">
                                    @includeIf('paginas.partials.producto-film-alimentos', ['producto' => $rp])
                                </div>					
                            @endforeach
                        </div>
                    </div>  
